...
// check if chip list files exist
if(params.tagSNPs_files) {
    params.tagSNPs_files.each { tagSNPs ->
        if (!file(tagSNPs.value).exists() && !file(tagSNPs.value).isFile()) exit 1, "|-- ERROR: Target VCF file ${tagSNPs.value} not found. Please check your config file."
    }
}
...
summary['Reference panels']  = params.tagSNPs_files.keySet().join(', ')
summary['Human reference genome']  = params.reference_genome
summary['Genetic map file']  = params.eagle_genetic_map
...
def mapFile_transform = { target_name, target_vcfFile, mapFile ->
...
targets_toImpute = mapFile_cha
        .flatMap{ it -> mapFile_transform(it) }
...
params.ref_panels.each { ref ->
    sample = params.ref_panels[ref.key].sampleFile
...
check_mismatch.toSortedList().val.each{ target_name, target_vcfFile, mapFile, warn, sumary ->
    ...
    if ( mismatch != 0 ) {
        ...
        check_mismatch_noMis << [ target_name, target_vcfFile, mapFile, warn, sumary]
    }
}
...
process target_qc {
    ...
    label "medium"
    input:
        set target_name, file(target_vcfFile), file(mapFile), file(warn), file(sumary) from check_mismatch_noMis
    output:
        set target_name, file(target_qc_vcfFile) into target_qc
        set target_name, file(mapFile) into target_qc_chunks
    script:
        base = file(target_vcfFile.baseName).baseName
        target_qc_vcfFile = "${base}_qc.vcf.gz"
        """
        bcftools view \
        ...
        bcftools norm \
            --rm-dup both \
            ${base}_noALT.vcf.gz \
            -Oz -o ${target_qc_vcfFile}
        rm -f ${base}_noALT.snp ${base}_noALT.vcf.gz
        """
}


def target_qc_transform = { target_name, target_qc_vcfFile ->
    target_qc_data = []
    params.tagSNPs_files.each { tag ->
        tagFile = file(tag.value)
        target_qc_data << [target_name, target_qc_vcfFile, tag.key, file(tagFile)]
    }
    return target_qc_data
}
target_qc_tag_data = target_qc
        .flatMap{ it -> target_qc_transform(it) }
...
process generate_chunks {
    ...
    label "small"
    input:
        set target_name, file(mapFile) from target_qc_chunks
    output:
...
"""
Split VCF into chunk
"""
all_chunks = generate_chunks.toSortedList().val
...
def extract_tag_from_target_vcf_transform = { target_name, target_vcfFile, tagName, tagFile, tag_target_vcfFile->
    chunks_datas = []
    all_chunks.each{ target_name_, chunk_file ->
        chunks = file(chunk_file).text.split()
        chunks.each{ chunk_data ->
            ...
            if (target_name == target_name_) {
                chunks_datas << [chrm, chunk_start, chunk_end, target_name, tagName, file(tag_target_vcfFile)]
            }
        }
    }
    return chunks_datas
}
target_qc_tag_chunks = extract_tag_from_target_vcf
        .flatMap{ it -> extract_tag_from_target_vcf_transform(it) }
...
process split_target_to_chunk {
    ...
    label "medium"
    input:
        set chrm, chunk_start, chunk_end, target_name, tagName, file(tag_target_vcfFile) from target_qc_tag_chunks
    output:
        set chrm, chunk_start, chunk_end, target_name, tagName, file(tag_target_vcfFile_chunk) into split_target_to_chunk
    script:
        base = file(tag_target_vcfFile.baseName).baseName
        tag_target_vcfFile_chunk = "${base}.chr${chrm}_${chunk_start}-${chunk_end}.vcf.gz"
        ...
        """
        tabix -f ${tag_target_vcfFile}
        bcftools view \
            --regions ${chrm}:${start}-${end} \
            -m2 -M2 -v snps \
            ${tag_target_vcfFile} \
            -Oz -o ${tag_target_vcfFile_chunk}
        """
}

def split_target_to_chunk_transform = { chrm, chunk_start, chunk_end, target_name, tagName, tag_target_vcf_chunk ->
    phasing_datas = []
    params.ref_panels.each { ref ->
        ref_m3vcf = sprintf(params.ref_panels[ref.key].m3vcfFile, chrm)
        ref_vcf = sprintf(params.ref_panels[ref.key].vcfFile, chrm)
        phasing_datas << [chrm, chunk_start, chunk_end, target_name, tagName, file(tag_target_vcf_chunk), ref.key, file(ref_vcf), file(ref_m3vcf), file(params.eagle_genetic_map)]
    }
    return phasing_datas
}

split_target_to_chunk_ref = split_target_to_chunk
        .flatMap{ it -> split_target_to_chunk_transform(it) }
...
process phase_target_chunk {
    ...
    label "bigmem"
    input:
        set chrm, chunk_start, chunk_end, target_name, tagName, file(target_vcfFile_chunk), ref_name, file(ref_vcf), file(ref_m3vcf), file(eagle_genetic_map) from split_target_to_chunk_ref
    output:
        set chrm, chunk_start, chunk_end, target_name, file("${file_out}.vcf.gz"), ref_name, file(ref_vcf), file(ref_m3vcf) into phase_target
    script:
        file_out = "${file(target_vcfFile_chunk.baseName).baseName}_${ref_name}_phased"
        base = "${file(target_vcfFile_chunk.baseName).baseName}"
        """
        bcftools query -f '%CHROM  %POS  %REF  %ALT\\n' ${target_vcfFile_chunk} > ${base}.snp
        nblines=\$(cat ${base}.snp | wc -l)
        if (( \$nblines > 0 ))
        then
            tabix -f ${ref_vcf}
            tabix -f ${target_vcfFile_chunk}
            eagle \
